...
resource "aws_subnet" "public" {
  ...
  tags = merge(local.common_tags, {
  Name = "${local.tag_prefix}-subnet-public-${count.index + 1}-${var.public_azs[count.index]}"
  })
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge(local.common_tags, {
  Name = "${local.tag_prefix}-private-subnet-${count.index + 1}-${var.private_azs[count.index]}"
  })
}
...
#   }
  tags = merge(local.common_tags, {
  Name = "${local.tag_prefix}-rtb-${count.index + 1}-private-${var.private_azs[count.index]}"
})
}
